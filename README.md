...
面向 K12 场景的 AI 数字教师平台（V3）。已完成：学生认证与账号状态安全（DISABLED 失效）、学生内容可见性边界、课程内容与阅读闭环、书库分页/搜索/筛选、对话 + RAG + SSE 幂等重放、章节完成与继续学习、错题讲解与复习/下一步行动、随堂测验（多题型/并发序号安全）、学习记忆与画像（含遗忘/否认不复活）、真实规则推荐、多 AI 教师风格、Admin 内容/风格/章节/知识资源管理、教学图片契约与渲染、长对话窗口化与 real 用量口径、统一存储抽象（本地/S3 兼容）、语音 ASR/TTS Provider 化、Postgres 任务 Worker（含卡住任务回收）、限流与 X-Request-ID/Prometheus 指标可观测性。

当前处于 **Phase 5 收口 + 本轮优化（T01–T26）验收**：本工作区已按 `tasks/plan.md` 完成技术回归、性能拆分（主包 -45%、Admin 独立成包）与 E2E（33 项，含手机 390/820/1280 视口），详见 `docs/plans/current-phase.md` 与 `tasks/acceptance/`。涉及真实付费模型、人工审校、真实用户招募与真实 S3/语音的项，如实列为待外部验收。
